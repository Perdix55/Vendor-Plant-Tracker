...
- **nodemailer**: Must be external. Has native-binding dependencies that esbuild cannot bundle cleanly.
- **connect-pg-simple**: Already external (pre-existing requirement).

**Why:** These packages use dynamic module loading internally, have native bindings, or have ESM/CJS mismatches that esbuild can't handle when bundling.
